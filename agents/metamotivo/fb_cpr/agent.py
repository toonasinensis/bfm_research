...
def _disable_torch_compile(fn):
    """No-op replacement for ``torch.compiler.disable`` in IsaacSim.

    Importing Dynamo through IsaacSim's bundled torch can fail before training
    even starts. The method still works correctly without the decorator.
    """
