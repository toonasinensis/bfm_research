...
class BaseHumEnvBenchWrapper:
    ...
    def act(
        ...
        if self.numpy_output:
            # we force the action to float in case we have a bfloat16 model
...
class RewardWrapper(BaseHumEnvBenchWrapper):
    ...
    make_env_fn: Any = make_humenv

    def reward_inference(self, task: str, **kwargs) -> torch.Tensor:
        # tmp workaround
        if hasattr(self.make_env_fn, "build"):
            cfg = self.make_env_fn.model_copy(update={"task": task})
            env, _ = cfg.build(1)
        else:
            env, _ = self.make_env_fn(task=task, **kwargs)
        if self.num_samples_per_inference >= len(self.inference_dataset) and hasattr(self.inference_dataset, "get_full_buffer"):
...
class GoalWrapper(BaseHumEnvBenchWrapper):
    def goal_inference(self, goal_pose: torch.Tensor | dict[str, torch.Tensor]) -> torch.Tensor:
...
class TrackingWrapper(BaseHumEnvBenchWrapper):
    def tracking_inference(self, next_obs: torch.Tensor | np.ndarray | dict[str, torch.Tensor]) -> torch.Tensor:
